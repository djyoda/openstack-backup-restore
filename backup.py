...
class Backup(object):
    ...
    def get_attached_volumes(self):
        ...
        :rtype: list
        """
        appLogger.info("Checking attached volumes for instance %s" % self.server_id)
        ...
        appLogger.debug("Attached volumes metadata %s: " % metadata)
        return metadata
    ...
    def create_snapshots(self, volumes):
        ...
        metadata = []
        for volume in volumes:
            appLogger.info("Creating snapshot for volume %s: " % volume.get("id"))
            ...
            while status == "creating":
                appLogger.debug("Snapshot: %s has creating status, wait for 2 sec ..." % snapshot.id)
                time.sleep(2)
                status = self.get_snapshot_status(snapshot.id)
            if status == "error":
                appLogger.error("Snapshot: %s has error state!" % snapshot.id)
                sys.exit()
            elif status == "available":
                appLogger.info("Snapshot: %s has been created." % snapshot.id)
                metadata.append({"created": snapshot.created_at, "id": snapshot.id,
                                 "size": snapshot.size,
                                 "display_name": volume.get("display_name")})
        appLogger.debug("Snapshots metatdat %s: " % metadata)
        return metadata
    ...
    def delete_snapshot(self, snapshot_id):
        ...
        appLogger.info("Deleting snapshot: %s" % snapshot_id)
    ...
    def delete_volume(self, volume_uuid):
        ...
        appLogger.info("Deleting volume: %s" % volume_uuid)
    ...
    def create_temp_volume(self, snaps_id):
        ...
        for snap_id in snaps_id:
            ...
            appLogger.info("Creating temporary volume: %s" % volume_name)
            vol = self.get_volume_metadata(temp_volume.id)
            status = vol.get("status")
            while status == "creating":
                appLogger.debug("Temporary volume: %s has creating status, "
                                "wait for 5 seconds ... " % temp_volume.id)
                time.sleep(5)
                vol = self.get_volume_metadata(temp_volume.id)
                status = vol.get("status")
                if status == "error":
                    appLogger.error("Temporary volume: %s has error status!" % temp_volume.id)
                    sys.exit()
                elif status == "available":
                    appLogger.info("Temporary volume: %s has been created." % temp_volume.id)
                    metadata.append(temp_volume.id)
        appLogger.debug("Temporary volumes id: %s" % metadata)
        return metadata
    ...
    def delete_temp_volume(self, volumes_uuid):
        ...
        for uuid in volumes_uuid:
            try:
                ...
                appLogger.error("There is no volume with uuid %s" % uuid)
                pass
            else:
                while True:
                    ...
                    if status == "available":
                        ...
                        appLogger.debug("Volume is not available, wait for 5 sec ...")
                        time.sleep(5)
    ...
    def delete_temp_snapshot(self, snaps):
        ...
        for snap in snaps:
            ...
            try:
                ...
                appLogger.error("There is no snapshot with uuid %s" % snapshot_id)
                pass
            else:
                while True:
                    ...
                    if status == "available":
                        ...
                        appLogger.debug("Snapshot is not available, wait for 5 sec ...")
                        time.sleep(5)
...
def main():
    ...
    appLogger.info("Creating backup for instance %s:  started." % server_id)
    ...
    appLogger.info("Creating backup finished.")
